...
FREQ_BINS = [0, 1, 2, 3, 4, 5] 

# cells for logging and testing purposes 
DETAILED_LOG_CELLS = {
    (75, 'PhoneNumber'), (9, 'ZipCode'), (5, 'MeasureCode'),
    (72, 'ZipCode'), (16, 'City'), (99, 'MeasureName'), (61, 'MeasureName') 
}

#generates features for the probabilistic model using unique names
class FeatureCompiler:
    ...
    #clears the features table before inserting new features
    ...
    def _load_data_and_stats(self):
        ...
        try:
            #fetches tuple data and calculates global counts
    ...
    def _build_dc_indexes(self):
        ...
        for tid, data in self.tuple_data_cache.items():
            ...
            if zip_val and zip_val != NULL_REPR_PLACEHOLDER: 
                self.dc_indexes['by_zip'][zip_val].append(tid)

            phone_val = data.get('PhoneNumber')

            if phone_val and phone_val != NULL_REPR_PLACEHOLDER: 
                self.dc_indexes['by_phone'][phone_val].append(tid)

            measure_val = data.get('MeasureCode')
            
            if measure_val and measure_val != NULL_REPR_PLACEHOLDER: 
                self.dc_indexes['by_measurecode'][measure_val].append(tid)
        ...
        for name, index in self.dc_indexes.items(): 
            logging.debug(f"  Index '{name}' size: {len(index)} keys")
    ...
    #generates log frequency features
    def generate_frequency_features(self):
        ...
        for (tid, attr), candidates in tqdm(self.cell_domains.items(), desc="Generating Freq Features"):
             ...
             for cand_val_str in candidates:
                 ...
                 bin_index = np.digitize(log_freq, FREQ_BINS, right=False) - 1
    ...
    #generates DC violation features
    def generate_dc_violation_features(self):
        ...
        for (tid, attr), candidates in tqdm(self.cell_domains.items(), desc="Checking DC Violations"):
            ...
            if not original_tuple_data: 
                continue

            log_this_cell_detail = (tid, attr) in DETAILED_LOG_CELLS
            if log_this_cell_detail: 
                logging.debug(f"[DC Detail Log] Cell ({tid}, {attr}): Candidates = {candidates}")
            ...
            for cand_val_str in candidates:
                 ...
                 if log_this_cell_detail: 
                     logging.debug(f"  [DC Detail Log] Checking Candidate: '{cand_val_str}'")
                 ...
                 for constraint in self.constraints:
                     ...
                     if violation_found:
                          ...
                          if log_this_cell_detail:
                              logging.debug(f"    [DC Detail Log] Candidate '{cand_val_str}' VIOLATES Constraint {constraint_id_str} (Partner TID: {violating_partner}). Added feature: {feature_name}")
                 ...
                 if log_this_cell_detail and not any(f[0]==tid and f[1]==attr and f[2]==cand_val_str and f[3].startswith("DC_VIOLATES_") for f in dc_features_list[-total_violations_found:]):
                     logging.debug(f"    [DC Detail Log] Candidate '{cand_val_str}' did NOT violate any checked constraints.")

        duration = time.time() - start_time
        logging.info(f"Checked {total_checks} potential constraint violations.")
        
        if skipped_constraints > 0: 
            logging.warning(f"Skipped DC checks for {skipped_constraints} constraint-candidate pairs...")
        
        logging.info(f"Generated {len(dc_features_list)} raw DC violation feature instances in {duration:.2f}s.")
        
        if logged_details_count > 0: 
            logging.info(f"Logged details for {logged_details_count} DC violation checks.")
    ...
    def compile_all(self):
        ...
        try:
            ...
            dc_feat_count = self.generate_dc_violation_features()
