...
FIXED_PRIOR_WEIGHT = -2.0 

# TiedLinear layer for sparse input
class TiedLinear(nn.Module):
    ...
    #initialize weights, setting the prior weight
    ...
    def forward(self, X_sparse, mask):
        ...
        b, d, f = X_sparse.size()
        W_col = self.weight.t().to(X_sparse.device)
...
class RepairModel:
    ...
    def predict_proba(self, X_pred_sparse, mask_pred, batch_size=256):
        ...
        self.model.eval()
        output_dim = self.output_dim; num_features = self.num_features
